																																																								...
																																																								if (($msg = preg_replace ('/%%user_id%%/i', c_ws_plugin__s2member_utils_strings::esc_refs ($user_id), $msg)))
																																																									{
																																																										if (is_array($fields) && !empty($fields))
																																																											foreach ($fields as $var => $val){ // Custom Registration/Profile Fields.
																																																												//Modified by Cultura Interactive: Jeisson Magallanes
																																																												$arr_unserialize = maybe_unserialize( $val );
																																																												$str_unserialize = "";
																																																												if( is_array( $arr_unserialize ) ){
																																																													$str_unserialize = implode( ', ', $arr_unserialize );
																																																												}else{
																																																													$str_unserialize = $val;
																																																												}
																																																												if (!($msg = preg_replace ('/%%' . preg_quote ($var, '/') . '%%/i', c_ws_plugin__s2member_utils_strings::esc_refs( $str_unserialize ), $msg)))
																																																													break; // Empty; we can stop here.
																																																											}
